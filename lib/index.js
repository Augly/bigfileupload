...
export class BigFileUpload {
  constructor(configObj = {}) {
    let _configObj = {
      ...
      //为一个函数接收三个参数(sliceFile,next)=>{}，该函数作用于上传文件切片
      uploadFun: (sliceFile, next,error) => { },
      //为一个函数接收一个参数(hashName)=>{}，此函数通知服务器端该文件所有切片已上传完毕，可以合并
      noticeFun: (fileName, hashName) => { },
      //为一个函数接收一个参数(fileName,hashName)=>{}，此函数验证该文件在服务器端已上传切片数量,该函数返回2个值，uploaded和uploadedList
